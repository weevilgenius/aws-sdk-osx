...
The `aws-sdk-macos` branch of this fork is a port of version 2.8.3 of the AWS Mobile SDK for iOS to macOS. The port is only missing `AWSLex`, `AWSPinpoint`, and the `Cognito Auth` frameworks. These frameworks are closly tied to iOS and UIKit. Considerable work would need to go into them to make them work. Once Apple supports UIKit on macOS (sometime this year we hope), this should be much easier.
